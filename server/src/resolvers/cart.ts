...
const cartResolver:Resolver = {
    ...
    Mutation :{
        addCart : (parent, {id}, {db}, info) =>{
            ...
            const targetProduct = db.products.find(item => item.id 
                === id)
        ...
        updateCart :(parent, { id, amount }, {db}) =>{
            ...
            if (existCartIndex < 0) { throw new Error('없는 데이터') }
            const newCartItem ={
                id,
                amount,
            }
    ...
    CartItem:{
        product:(cartItem,args,{db}) => db.products.find((product:any) => product.id === 
            cartItem.id)
    }
}
